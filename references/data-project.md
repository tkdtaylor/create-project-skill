...
4. If token was saved: D6 will read it from `/tmp/.create-project-pat`. Do not try to echo or use the token value again — it is already on disk.

   If saving failed: tell the user to run `gh auth refresh -h github.com -s write:personal_access_tokens` and retry, or create a fine-grained PAT manually at https://github.com/settings/personal-access-tokens/new (Repository access: this repo only; Permissions: Contents read/write, Metadata read-only). D6 will prompt for it.
...
Same as tech — read host git identity, check for saved PAT from D5, create `.env` with `ANTHROPIC_API_KEY`, `GIT_TOKEN`, `GIT_REMOTE_URL`, `GIT_USER_NAME`, `GIT_USER_EMAIL`.
...
If `GIT_TOKEN` is non-empty: create `.env` with token filled in, tell user to fill in `ANTHROPIC_API_KEY`.

If no token: ask about remote, create `.env` with blanks for token and remote URL.
