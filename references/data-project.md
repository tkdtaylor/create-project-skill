...
3. **Tell the user to create a fine-grained access token themselves** — programmatic PAT creation via `gh api` requires a specific auth scope that usually isn't granted, and it silently fails more often than it works.
   ...
   > 7. In step D6 you'll be asked to paste it into `.env`

   The token never gets written into the repo — D6 puts it in `.env` (which is gitignored) and the container entrypoint loads it into git's credential helper at runtime.
